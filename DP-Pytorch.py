...
# define the noise values to plot
noises = [0.001, 0.01, 0.9, 4]

# create a list of colors to use for the different noise values
colors = ['g', 'b', 'm', 'c']

# initialize a list to store the losses for each noise value
all_losses = []

# loop over the different noise values
for noise, color in zip(noises, colors):
    # create a new optimizer with the current noise value
    optimizer = DP_SGD(model.parameters(), lr=0.1, max_norm=0.01, stddev=2.0, noise=noise)
    ...
    # initialize the losses and accuracies for the current noise value
    losses = []
    accuracies = []

    # loop over the epochs and batches as before
    ...
    # append the losses for the current noise value to the list of all losses
    all_losses.append(losses)

    # plot the losses for the current noise value
    plt.plot(losses, color=color, label=f'Noise={noise}')

# plot the legend
plt.legend()

# plot the title and labels
plt.title("Loss for Different Noise Values")
